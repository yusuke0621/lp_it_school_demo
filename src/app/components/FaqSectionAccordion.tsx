...
Growbase は「なにが分からないかも分からない」という方のためのカリキュラム設計になっているため、IT・デザインの知識ゼロからでも安心して学べます。`,
  },
  {
    question: "どれくらいの期間で転職できますか？",
    answer: `人によって進度は異なりますが、平均して6〜12ヶ月での転職成功を目標としています。
    
        Growbase では「最短での就職」をゴールにせず、"自分に合ったペースで学び、納得して働ける転職"を大切にしています。
  ...
  },
  {
    ...
    answer: `受講料はプランやサポート内容によって異なりますが、基本的に月額制 or 分割払いにも対応しています。
      ...
      ※Growbase では転職成功後に企業からの紹介料で収益化するモデルのため、学習者の自己負担は極力少なく抑えています。
...
Growbase では、「無理なく続けられる」ことを大前提にカリキュラム設計をしています。
...
「フルタイムで働きながら」「家事や育児と両立しながら」という方も多く、メンターチャットサポートが継続をサポートします。`,
  },
  {
    ...
    answer: `ご安心ください。Growbase では、最初に「キャリア相談」や「適性セッション」を行い、あなたの興味・価値観・得意不得意に応じて、どちらが合っているかを一緒に見つけていきます。
    また、実際に両方に触れてみる体験型の講座もあるので、学びながら選ぶこともできます。`,
  },
  {
    question: "他のスクールと何が違うんですか？",
    answer: `Growbase の特徴は、「未経験者が"ちゃんと続けられて、ちゃんと仕事にできる"」仕組みがあることです。
  ...
  },
  {
    ...
    answer: `基本はオンラインですが、「Growbase カフェ（勉強会・交流会）」として対面イベントも実施しています。
...
export default function FAQSection() {
  ...
  function highlight(text: string, keywords: string[]) {
    ...
    keywords.forEach((word) => {
      ...
      highlighted = highlighted.replace(
        regex,
        '<span class="text-pink-primary font-semibold">$1</span>'
      );
    });
  ...
  return (
    <section id="faq" className="px-4 md:py-20 md:px-30 bg-black">
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 max-w-3xs">
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#ff2e9f] to-[#9916ff] font-bold text-base">
                    Q
                  </span>
                  <p className="text-xs md:text-base font-semibold md:whitespace-nowrap text-black">
                    {faq.question}
                  </p>
                </div>
                <div
                  className={`w-6 h-6 rounded-full ${openIndex === idx ? "bg-gray-500" : "bg-gradient-to-r from-[#ff2e9f] to-[#9916ff"} flex items-center justify-center text-white text-base font-bold`}
                >
                  {openIndex === idx ? "ー" : "+"}
                </div>
              </div>
                    ...
                    {faq.answer.split("\n").map((line, i) => (
                      <p key={i}>
                        {highlight(line, [
                          ...
                          "メンター・勉強会・コミュニティ",
                          "キャリア相談",
                          "「Growbase カフェ（勉強会・交流会）」として対面イベントも実施",
                        ])}
                      </p>
                    ))}
                  </div>
